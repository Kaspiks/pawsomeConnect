    ...
    <div class="container mx-auto p-4">
        ...
        <ul class="space-y-4">
            @foreach($post->comments as $comment)
                <li class="bg-gray-100 p-4 rounded-md {{ $comment != $post->comments->first() ? 'mt-4' : '' }} {{ $comment == $post->comments->last() ? 'mb-4' : ''}}">
                    <p>{{ $comment->content }}</p>
                    @if ($comment->user)
        ...
        @auth
            <form action="{{ route('comments.store', $post->id) }}" method="POST" class="mt-6 space-y-4">
                ...
                <button type="submit" class="bg-gray-800 hover:bg-gray-700 text-white font-bold py-2 px-4 rounded mt-4">Submit Comment</button>
            </form>
        @endauth
    </div>
</body>
